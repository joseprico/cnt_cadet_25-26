// api/importar-acta.js (Vercel/Netlify style)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    $('table').each((_, tbl) => {
      ...
      // Guarda només blocs amb jugadors
      if (jugadors.length) blocs.push({ nomEquip, jugadors });
    });

    if (!blocs.length) return res.status(422).send('No s’han trobat taules de jugadors a l’acta');

    // Generalment hi ha dos blocs: local i visitant
